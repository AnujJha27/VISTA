...
| `FormalBindingCandidate`s (site_count, operator_form, long_range_pairs, xc_form) | `DFTCapabilityPlugin.formal_binding_candidates` | `evidence_refs` → IR provenance nodes (`long_range_pairs` carries none -- it is `specified_interface`, not artifact-grounded) | inferred/specified, independently revalidated |
...
| Long-range pair specification (`interface_contract.long_range_pairs`) | N/A — same as the output role mapping: `specified_interface`, not derived; a domain expert changing which pairs count as long-range is a legitimate interpretation change, always reflected as a different `ir_sha256`/`package_sha256`, never silently upgraded to `artifact_grounded` and never claimed to be physically correct in the first place (research-soundness correction) | — | Yes (hashed into `package_sha256`) | No | code inspection: `dft_capability_plugin.py::_long_range_pairs_syntax`; `docs/structural-v2/STRUCTURAL_CAPABILITY_CHECKS.md`'s "Provisional long-range-coupling correction" |
