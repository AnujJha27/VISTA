...
| Discharge of 3 structural premises | `rfl` against `guaranteedSelfAdjoint`/`canRepresentNonLocal`/`xcSupportsDiscontinuity` (`StructuralV2.lean`) | kernel-checked by `verify_structural_certificate` | formally checked (real Bool computation, not vacuous — see below) |
...
| `FormalBindingCandidate`s (site_count, operator_form, xc_form) | `DFTCapabilityPlugin.formal_binding_candidates` | `evidence_refs` → IR provenance nodes | inferred, independently revalidated |
